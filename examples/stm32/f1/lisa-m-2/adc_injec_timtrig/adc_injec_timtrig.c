...
int main(void)
{
	...
	while (1) {
		/*
		 * Since the injected sampling is triggered by the timer, it
		 * gets updated automatically, we just need to periodically
		 * read out the value. It would be better to check if the JEOC
		 * bit is set, and clear it following so that you do not read
		 * the same value twice, especially for a slower sampling rate.
		 */
