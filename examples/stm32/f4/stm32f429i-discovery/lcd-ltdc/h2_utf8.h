...
static inline
const char *
utf8_find_character_index(int index, const char *text_start, const char *text_end, int *index_rest) {
	const char *result;
	if (index<0) {
		result = text_end;
		while (index                     && result!=text_start) {
			index++;
			result--;
			while (((*result)&192)==128  && result!=text_start) { // skip next utf8-character (secondary bytes)
				result--;
			}
		}
	} else {
		...
		while (index                     && result!=text_end) {
			index--;
			result++;
			while (((*result)&192)==128  && result!=text_end) { // skip last utf8-character (secondary bytes)
				result++;
			}
		}
	}
...
//untestet!
static inline
const char * utf8_find_character_in_string(const char chr, const char *text_start, const char *text_end) {//, int *index_rest) {
...
int utf8_find_pointer_diff(const char *text_start, const char *text_end) {
	...
	while (text_start<text_end) {
		if (!*text_start) return -1;
		
		text_start++;
		diff++;
		while (((*text_start)&192)==128  && text_start<text_end) // skip last utf8-character (secondary bytes)
			text_start++;
	}
	return diff;
}
