...
void ili9341_init(uint16_t *layer1_surface_buffers[], uint16_t *layer2_surface_buffers[]) {
	...
	//ili9341_opts.orient = ILI9341_ORIENTATION_0_DEGREE;
	...
	for (i=0; i<ILI9341_BUFFERS_PER_LAYER; i++) {
		ili9341_opts.layer1_surface_buffers[i] = layer1_surface_buffers[i];
		ili9341_opts.layer2_surface_buffers[i] = layer2_surface_buffers[i];
	}
...
void ili9341_init_pins(void) {
	...
	//WRX pin
	gpio_mode_setup(ILI9341_WRX_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, ILI9341_WRX_PIN);
	gpio_set_output_options(ILI9341_WRX_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_100MHZ, ILI9341_WRX_PIN);
	//SPI CS pin
	gpio_mode_setup(ILI9341_CS_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_NONE, ILI9341_CS_PIN);
	gpio_set_output_options(ILI9341_CS_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_100MHZ, ILI9341_CS_PIN);
	//Reset pin
	gpio_mode_setup(ILI9341_RST_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLUP, ILI9341_RST_PIN);
	gpio_set_output_options(ILI9341_RST_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_100MHZ, ILI9341_RST_PIN);
	// SPI5 clk and mosi
...
void lcd9341_init_ltdc(void) {
	...
	/*rcc_ltdc_set_clock_divr(RCC_DCKCFGR_PLLSAIDIVR_DIVR_4); // << this results in tearing */
	rcc_ltdc_set_clock_divr(RCC_DCKCFGR_PLLSAIDIVR_DIVR_8); // << this seem ideal.. */
	/*rcc_ltdc_set_clock_divr(RCC_DCKCFGR_PLLSAIDIVR_DIVR_16); << this results slightly moving screen */
...
void ili9341_init_layers(void) {
    ...
    /* To use blending in without alpha in RGB565 is actually a bit hard to do :)
     */
	...
	ltdc_set_blending_factors(LTDC_LAYER_1, LTDC_LxBFCR_BF1_PIXEL_ALPHA_x_CONST_ALPHA, LTDC_LxBFCR_BF2_PIXEL_ALPHA_x_CONST_ALPHA);
	/* Here we say to blend layer to with layer 1 according to their
	 * alpha channels. I don't know at the moment if color keying is affected by this
	 */
    /* top layer */
	ltdc_set_blending_factors(LTDC_LAYER_2, LTDC_LxBFCR_BF1_PIXEL_ALPHA_x_CONST_ALPHA, LTDC_LxBFCR_BF2_PIXEL_ALPHA_x_CONST_ALPHA);
	/* note: there's also the background color (we deal with 3 layers in total) */


	/* TODO WARNING!! Color key (ILI9341_LAYER2_COLOR_KEY) defined for layer 2!!!
	 ...
	 */
	ltdc_set_color_key(LTDC_LAYER_2,
			(ltdc_get_rgb888_from_rgb565(ILI9341_LAYER2_COLOR_KEY)>>16)&0xff,
			(ltdc_get_rgb888_from_rgb565(ILI9341_LAYER2_COLOR_KEY)>> 8)&0xff,
			(ltdc_get_rgb888_from_rgb565(ILI9341_LAYER2_COLOR_KEY)>> 0)&0xff
		);
    ...
    */
	...
	ltdc_set_fbuffer_address(LTDC_LAYER_1, (uint32_t)ili9341_opts.layer1_surface_buffers[0]);

    /* Start Address configuration : the LCD Frame buffer is defined on SDRAM w/ Offset */  
	ltdc_set_fbuffer_address(LTDC_LAYER_2, (uint32_t)ili9341_opts.layer2_surface_buffers[0]);
