...
typedef struct {
	...
	uint8_t         charwidth;
	uint16_t        char_count;
	const char_t   *chars;     // pointer to chars array (sorted by utf8_value)
	const uint32_t *chars_data; // pointer to chars_data array
} font_t;
...
font_get_char_index(uint32_t utf8_value, const font_t *font) {
	...
	while (1) {
		...
		if (utf8_value < cc) e = i;
		else
		if (utf8_value > cc) s = i;
		else {
			return &font->chars[i];
		}
		if (i==j) return NULL;
